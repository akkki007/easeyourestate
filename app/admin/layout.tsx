...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  const isAdmin = !isLoginPage && isHydrated && !!token && user?.role === "admin";
  ...
  // Auth loading
  if (!isHydrated || !token || !user || user.role !== "admin") return null;
  ...
  return (
    <div className="min-h-screen bg-gray-950">
        ...
        <div className="h-16 flex items-center px-5 border-b border-gray-800">
          <Link href="/admin" className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[10px] text-gray-500">easeyourestate</p>
            </div>
          </Link>
        </div>
        ...
        <div className="border-t border-gray-800 p-4">
          <div className="flex items-center gap-3 mb-3">
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-xs text-gray-500 truncate">admin@easeyourestate.com</p>
            </div>
          </div>
          <button
      ...
      <div className="lg:pl-64 min-h-screen">
        {/* Top bar */}
        <header className="h-14 bg-gray-900/50 backdrop-blur-sm border-b border-gray-800 flex items-center justify-between px-4 md:px-6 sticky top-0 z-20">
          <div className="flex items-center">
            <button
              onClick={() => setSidebarOpen(true)}
              className="lg:hidden p-2 -ml-2 rounded-lg text-gray-400 hover:bg-gray-800 transition-colors"
            >
              <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="1.5">
                <path strokeLinecap="round" strokeLinejoin="round" d="M4 6h16M4 12h16M4 18h16" />
              </svg>
            </button>
            <h1 className="text-sm font-medium text-white ml-2 lg:ml-0">
          ...
          <div className={`flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[11px] font-medium transition-colors ${
            ...
            <span className={`w-1.5 h-1.5 rounded-full ${
              realtime.connected ? "bg-green-400 animate-pulse" : "bg-gray-600"
            }`} />
            {realtime.connected
