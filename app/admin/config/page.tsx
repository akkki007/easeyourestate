    ...
    <main className="p-6 max-w-3xl">
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">Manage feature toggles and system settings</p>
        </div>
        <button
          onClick={handleSave}
            ...
            {item.type === "toggle" ? (
              <button
                ...
                className={`relative w-11 h-6 rounded-full transition-colors shrink-0 ${
                  item.value ? "bg-accent" : "bg-gray-700"
                }`}
              >
                <span
              ...
              <input
                ...
                min={0}
                className="w-20 px-3 py-1.5 rounded-lg bg-gray-800 border border-gray-700 text-white text-sm text-center focus:border-accent focus:outline-none shrink-0"
              />
            )}
          </div>
        ))}
      ...
      <div className="mt-8 bg-gray-900/50 border border-gray-800 rounded-xl p-4">
        <p className="text-xs text-gray-500">
          Note: Configuration changes are currently UI-only. Backend persistence via <code className="text-gray-400">/api/admin/config</code> will be implemented with database-backed settings in a future update.
        </p>
      </div>
    </main>
  );
